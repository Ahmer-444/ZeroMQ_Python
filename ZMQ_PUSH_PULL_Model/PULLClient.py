...
class ZMQ_PULLClient(threading.Thread):
    ...
    def run(self):
        context = zmq.Context()
        print "Starting High-latency (PULL) Client"
        socket = context.socket(zmq.PULL)
        socket.connect ("tcp://" + self._meg_device_ip + ":%s" % self._port_low_latency)
        print "Started, listening for messages (PULL) from: " + self._meg_device_ip + "\n"
        while True:
            sleep(0.02)
            message = socket.recv()
            print message
